...
export default function CartOrderView({
  ...
  const [shippingMethod, setShippingMethod] = useState<'free' | 'dhaka' | 'outside'>('free');
  ...
  const shippingLabel = useMemo(() => {
    if (shippingMethod === 'free') return 'ডিজিটাল / ফ্রি শিপিং — লোকেশন লাগবে না';
              ...
              {isShippingDropdownOpen && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden py-1">
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 text-xs sm:text-sm font-medium flex items-center justify-between hover:bg-gray-50 ${shippingMethod === 'free' ? 'bg-red-50/70 text-[#df2d4d] font-bold' : 'text-gray-700'}`}
                  >
                    <span>ডিজিটাল / ফ্রি শিপিং — লোকেশন লাগবে না</span>
